...
let package = Package(
    ...
    products: [
        ...
        .library(name: "tvOSAVPlayer-GoogleAds-tvos-tvos", targets: ["s2s_sdk_tvos_avplayer_googleAds"]),
        .library(name: "tvOSBitmovin-tvos-tvos", targets: ["s2s_sdk_tvos_bitmovin"]),
        .library(name: "tvOSAgentOnly-tvos-tvos", targets: ["s2s_sdk_tvos_agent_only"])
    ],
    targets: [
        .binaryTarget(
